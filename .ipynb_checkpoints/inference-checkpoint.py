...
if __name__ == "__main__":
    ...
    if num_valid < num_points:
        print(f"[Warning] Only {num_valid} valid pixels found; using all of them.")
        selected = np.arange(num_valid)
    else:
        selected = np.random.choice(num_valid, size=num_points, replace=False)
    ...
    with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        if viz:
            viser.visualize(video=video[None],
                                ...
                                visibility=vis_pred[None],filename=f"{args.video_name}")
        ...
        np.savez(os.path.join(out_dir, f'{args.video_name}.result.npz'), **data_npz_load)
