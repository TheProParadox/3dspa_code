...
[TAPVid-3D Website](https://tapvid3d.github.io/) | [TAPVid-3D Paper](https://arxiv.org/abs/2406.08729) | [Colab to Visualize Samples](https://colab.research.google.com/github/google-deepmind/tapnet/blob/main/colabs/tapvid3d_visualization.ipynb)
...
- [Aria Digital Twin](https://about.meta.com/realitylabs/projectaria/tools/digital-twin/)
- [Waymo Open Dataset](https://waymo.com/open/terms/)
- [Panoptic Studio](http://domedb.perception.cs.cmu.edu/)
...
For more details, including the performance of multiple baseline 3D point tracking models on the benchmark, please see the paper: [TAPVid-3D: A Benchmark for Tracking Any Point in 3D](https://arxiv.org/abs/2406.08729).
...
Because of license restrictions in distribution of the underlying source videos for Aria Digital Twin, you will need to accept their licence terms and download the ADT dataset by first getting the cdn json file from [Project Aria Explorer](https://www.projectaria.com/tools/explorer/), and downloading the ADT main_vrs, main_groundtruth, segmentation and depth files with:
...
You can view samples of the dataset, using a public Colab demo: [TAPVid-3D Colab Visualization](https://colab.research.google.com/github/google-deepmind/tapnet/blob/main/colabs/tapvid3d_visualization.ipynb).
...
## Quick Download: Minival Split

To generate the minival split, you can use the provided script:

```bash
bash scripts/download_tapvid3d_minival.sh
```

This script will:
1. Check if `tapnet` is installed, and install it if needed
2. Provide instructions for generating the minival split for each dataset source

**Note:** The minival split needs to be generated from source videos. For the easiest start, you can generate the Panoptic Studio minival split, which doesn't require additional video downloads (unlike ADT which requires Project Aria Explorer access).

**Quick start for Panoptic Studio minival:**
```bash
# Install tapnet if not already installed
pip install "git+https://github.com/google-deepmind/tapnet.git#[tapvid3d_eval,tapvid3d_generation]"

# Generate minival split
python3 -m tapnet.tapvid3d.annotation_generation.generate_pstudio --split=minival --output_dir=./tapvid3d_dataset
```
